...
系统采用 **双模式检测** 架构，结合规则引擎的稳定性和 LLM 智能体的灵活性：
...
│         ▼                  ▼  ▼                  ▼  ▼                ▼   │
│  ┌────────────┐    ┌────────────┐    ┌────────────┐    ┌───────────┐        │
│  │Classified │    │ Sensitive  │    │Restricted  │    │Credential │        │
│  │  Agent    │    │   Agent    │    │   Agent    │    │   Agent   │        │
│  │ 涉密检测   │    │ 敏感信息检测│    │ 受限内容检测│    │ 凭证检测  │        │
│  └────────────┘    └────────────┘    └────────────┘    └───────────┘        │
│         │                  │                  │                │               │
│         ▼                  ▼                  ▼                ▼               │
│  ┌─────────────────────────────────────────────────────────────────┐        │
│  │                         Skill 层 (检测规则)                      │        │
│  │  classified_ | sealed_ | sensitive_ | privacy_ | restricted_  │        │
│  │  credential_ | infra_                                           │        │
│  └─────────────────────────────────────────────────────────────────┘        │
└─────────────────────────────────────────────────────────────────────────────┘
                                     │
...
| 模式 | 说明 | 适用场景 |
|------|------|----------|
| **规则引擎** | 执行所有预定义 Skills，基于正则表达式和关键词库检测 | 快速检测、已知模式 |
| **AI智能体** | LLM 智能决策，先分析文本选择需要检测的类别和 Skills | 复杂场景、未知模式 |

### LLM 智能决策流程

1. **Master Agent** 分析文本语义，决定需要检测的 Category
...
│   ┌────────────────────────────────────────────────────────────────┐        │
│   │                    Master Agent 加载规则                       │        │
│   │   • 启动时加载 SkillStatus.ACTIVE 的规则                       │        │
│   │   • 与预定义 Skills 一起执行检测                                │        │
│   └────────────────────────────────────────────────────────────────┘        │
│                                                                             │
│   评估标准:                                                                  │
│   • F1 >= 0.8  → ACTIVE (加入检测流程)                                      │
│   • 0.5 <= F1 < 0.8 → TESTING (待测试)                                      │
│   • F1 < 0.3 → DISCARDED (丢弃)                                             │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
│   │   ├── learning/                   # 边学边用模块
│   │   │   ├── learning_agent.py       # 规则学习智能体
│   │   │   ├── evaluation_agent.py     # 规则评估智能体
...
├── logs/                               # 检测日志目录
└── test_files/                         # 测试文件目录
    └── aiso_system_architecture.drawio  # 系统架构图
```
...
```
┌─────────────┐    ┌─────────────┐    ┌─────────────┐    ┌─────────────┐
│  LLM 检测   │ →  │   Learning  │ →  │  Evaluation │ →  │   规则激活   │
│  发现问题   │    │    Agent    │    │    Agent    │    │  加入检测    │
│             │    │  提取规则    │    │  F1 评估     │    │             │
└─────────────┘    └─────────────┘    └─────────────┘    └─────────────┘
```
...
| 状态 | 说明 | 阈值 |
|------|------|------|
| testing | 待测试 | 0.5 <= F1 < 0.8 |
| active | 已激活，加入检测 | F1 >= 0.8 |
| discarded | 已丢弃 | F1 < 0.3 |

### 边学边用流程详解

1. **LLM 检测发现问题**：AI智能体模式检测到新的安全问题模式
2. **Learning Agent 学习**：从检测结果中提取规则，生成 LearnedSkill
3. **Evaluation Agent 评估**：使用测试样本评估规则效果，计算 F1 分数
4. **规则激活**：F1 >= 0.8 的规则自动标记为 ACTIVE
5. **Master Agent 加载**：系统启动时加载所有 ACTIVE 规则
6. **参与检测**：learned_skills 与预定义 Skills 一起执行检测
...
- ✅ 边学边用规则自动加入检测流程（F1 >= 0.8 激活）
- ✅ 规则自动评估（F1 分数阈值激活/丢弃）
- ✅ 多文件并发处理
- ✅ OCR 公章检测
- ✅ 综合处理建议
- ✅ 检测日志详情
- ✅ 支持多种 LLM 提供商（OpenAI/阿里云/百度）
- ✅ 模块化设计，便于扩展新的检测技能
